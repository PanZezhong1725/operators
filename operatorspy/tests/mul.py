...
def test(
    ...
    print(
        f"Testing Mul on {torch_device} with c_shape:{c_shape} a_shape:{a_shape} b_shape:{b_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    assert torch.allclose(c, ans, atol=1e-6, rtol=1e-3)
