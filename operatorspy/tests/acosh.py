...
class AcoshDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopAcoshDescriptor_t = POINTER(AcoshDescriptor)


def acosh(x):
    if PROFILE:
        ans = torch.acosh(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Acosh on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopAcoshDescriptor_t()

    check_error(
        lib.infiniopCreateAcoshDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopAcosh(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopAcosh(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=1e-3, equal_nan=True)
    check_error(lib.infiniopDestroyAcoshDescriptor(descriptor))
...
if __name__ == "__main__":
    ...
    lib.infiniopCreateAcoshDescriptor.argtypes = [
        infiniopHandle_t,
        POINTER(infiniopAcoshDescriptor_t),
        infiniopTensorDescriptor_t,
        infiniopTensorDescriptor_t,
    ]
