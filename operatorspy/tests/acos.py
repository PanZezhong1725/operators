...
class AcosDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopAcosDescriptor_t = POINTER(AcosDescriptor)


def acos(x):
    if PROFILE:
        ans = torch.acos(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Acos on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopAcosDescriptor_t()

    check_error(
        lib.infiniopCreateAcosDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopAcos(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopAcos(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=1e-3, equal_nan=True)
    check_error(lib.infiniopDestroyAcosDescriptor(descriptor))
