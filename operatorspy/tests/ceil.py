...
class CeilDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopCeilDescriptor_t = POINTER(CeilDescriptor)
...
def test(
    ...
    print(
        f"Testing Ceil on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopCeilDescriptor_t()

    check_error(
        lib.infiniopCreateCeilDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopCeil(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopCeil(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=1e-3, equal_nan=True)
    check_error(lib.infiniopDestroyCeilDescriptor(descriptor))
