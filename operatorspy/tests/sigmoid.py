...
def sigmoid(x):
    if PROFILE:
        ans = torch.sigmoid(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Sigmoid on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopSigmoid(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopSigmoid(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
