...
class CoshDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopCoshDescriptor_t = POINTER(CoshDescriptor)


def cosh(x):
    if PROFILE:
        ans = torch.cosh(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Cosh on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopCoshDescriptor_t()

    check_error(
        lib.infiniopCreateCoshDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopCosh(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopCosh(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=0, equal_nan=True)
    check_error(lib.infiniopDestroyCoshDescriptor(descriptor))
