...
class SinDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopSinDescriptor_t = POINTER(SinDescriptor)


def sin(x):
    if PROFILE:
        ans = torch.sin(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Sin on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopSinDescriptor_t()

    check_error(
        lib.infiniopCreateSinDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopSin(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopSin(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=1e-4, rtol=1e-2, equal_nan=True)
    check_error(lib.infiniopDestroySinDescriptor(descriptor))
