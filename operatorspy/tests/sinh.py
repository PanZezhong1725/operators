...
class SinhDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopSinhDescriptor_t = POINTER(SinhDescriptor)


def sinh(x):
    if PROFILE:
        ans = torch.sinh(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Sinh on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopSinhDescriptor_t()

    check_error(
        lib.infiniopCreateSinhDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopSinh(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopSinh(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=0, equal_nan=True)
    check_error(lib.infiniopDestroySinhDescriptor(descriptor))
