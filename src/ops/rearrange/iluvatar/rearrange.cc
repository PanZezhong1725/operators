...
infiniopStatus_t cudaCreateRearrangeDescriptor(IluHandle_t handle,
    ...
    if (ndim == 2) {
        ...
        for (int i = ndim - 3; i >= 1; --i) {
            if (static_cast<int64_t>(dst->shape[i]) * dst->strides[i] != dst->strides[i - 1] || static_cast<int64_t>(src->shape[i]) * src->strides[i] != src->strides[i - 1]) {
                return STATUS_BAD_TENSOR_STRIDES;
            }
        }
