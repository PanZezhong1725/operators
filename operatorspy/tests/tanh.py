...
class TanhDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopTanhDescriptor_t = POINTER(TanhDescriptor)


def tanh(x):
    if PROFILE:
        ans = torch.tanh(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Tanh on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopTanhDescriptor_t()

    check_error(
        lib.infiniopCreateTanhDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopTanh(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopTanh(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=0, equal_nan=True)
    check_error(lib.infiniopDestroyTanhDescriptor(descriptor))
...
if __name__ == "__main__":
    ...
    lib.infiniopCreateTanhDescriptor.argtypes = [
        infiniopHandle_t,
        POINTER(infiniopTanhDescriptor_t),
        infiniopTensorDescriptor_t,
        infiniopTensorDescriptor_t,
    ]
