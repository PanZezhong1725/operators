...
class AsinhDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopAsinhDescriptor_t = POINTER(AsinhDescriptor)


def asinh(x):
    if PROFILE:
        ans = torch.asinh(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Asinh on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopAsinhDescriptor_t()

    check_error(
        lib.infiniopCreateAsinhDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopAsinh(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopAsinh(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=1e-3, equal_nan=True)
    check_error(lib.infiniopDestroyAsinhDescriptor(descriptor))
...
if __name__ == "__main__":
    ...
    lib.infiniopCreateAsinhDescriptor.argtypes = [
        infiniopHandle_t,
        POINTER(infiniopAsinhDescriptor_t),
        infiniopTensorDescriptor_t,
        infiniopTensorDescriptor_t,
    ]
