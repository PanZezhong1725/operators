...
class NegDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopNegDescriptor_t = POINTER(NegDescriptor)
...
def test(
    ...
    print(
        f"Testing Neg on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )
    ...
    descriptor = infiniopNegDescriptor_t()

    check_error(
        lib.infiniopCreateNegDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopNeg(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopNeg(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=1e-3, equal_nan=True)
    check_error(lib.infiniopDestroyNegDescriptor(descriptor))
...
if __name__ == "__main__":
    ...
    lib.infiniopCreateNegDescriptor.argtypes = [
        infiniopHandle_t,
        POINTER(infiniopNegDescriptor_t),
        infiniopTensorDescriptor_t,
        infiniopTensorDescriptor_t,
    ]
