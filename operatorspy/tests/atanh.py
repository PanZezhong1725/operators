...
class AtanhDescriptor(Structure):
    _fields_ = [("device", c_int32)]


infiniopAtanhDescriptor_t = POINTER(AtanhDescriptor)


def atanh(x):
    if PROFILE:
        ans = torch.atanh(x).to(x.dtype)
...
def test(
    ...
    print(
        f"Testing Atanh on {torch_device} with tensor_shape:{tensor_shape} dtype:{tensor_dtype} inplace: {inplace.name}"
    )

    x = torch.rand(tensor_shape, dtype=tensor_dtype).to(torch_device) * 3 - 1.5
    ...
    descriptor = infiniopAtanhDescriptor_t()

    check_error(
        lib.infiniopCreateAtanhDescriptor(
            handle,
    ...
    for i in range(NUM_PRERUN if PROFILE else 1):
        check_error(lib.infiniopAtanh(descriptor, y_tensor.data, x_tensor.data, None))
    if PROFILE:
        start_time = time.time()
        for i in range(NUM_ITERATIONS):
            check_error(
                lib.infiniopAtanh(descriptor, y_tensor.data, x_tensor.data, None)
            )
        elapsed = (time.time() - start_time) / NUM_ITERATIONS
        print(f"    lib time: {elapsed :6f}")

    assert torch.allclose(y, ans, atol=0, rtol=1e-3, equal_nan=True)
    check_error(lib.infiniopDestroyAtanhDescriptor(descriptor))
