...
void random_sample_cpu_f16(Tensor source, Tensor indices, Tensor index, float random, float topp, int topk) {
    ...
    auto indices_ = reinterpret_cast<int const *>(indices.data);
    auto index_ = reinterpret_cast<int *>(index.data);
    float p = random * std::min(topp * f16_to_f32(source_[voc - 1]), f16_to_f32(source_[topk - 1]));
